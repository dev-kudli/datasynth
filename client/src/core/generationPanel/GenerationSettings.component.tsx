...
import { getCompletedDataString } from '~store/packets/packets.selectors';
...
const GenerationSettingsPanel = ({
	...
	const completedDataString = useSelector(getCompletedDataString);

	let buttonLabel = i18n.generate;
	let actionButtonClick = onGenerate;
	let actionButtonDisabled = !!error;

	const uploadButtonLabel = i18n.upload;
	// let actionButtonClick = onGenerate;
	let uploadButtonDisabled = true;

	const uploadButtonClick = async () => {
		const UPLOAD_URL = "http://localhost:3001/upload-w2";
	
		const w2Xml = completedDataString;
		console.log('w2xml is: ', w2Xml);
